...
Dialog:BuyPetsDialog(playerid, response, listitem, inputtext[]) {
    if(response) {
        if(GetMoney(playerid) < PET_SHOP[listitem][PET_PRICE]) {
            SendClientMessage(playerid, COLOR_LIGHTRED, "Você não possui US$ %s em mãos para comprar um %s.", FormatNumber(PET_SHOP[listitem][PET_PRICE]), PET_SHOP[listitem][PET_NAME]);
        ...
        SendClientMessage(playerid, COLOR_LIGHTRED, "Você comprou um %s e pagou US$ %s nele.", PET_SHOP[listitem][PET_NAME], FormatNumber(PET_SHOP[listitem][PET_PRICE]));
