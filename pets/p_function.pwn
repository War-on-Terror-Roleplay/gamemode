...
PetSpawn(playerid) {
    if(PetData[playerid][petSpawn]) return SendClientMessage(playerid, COLOR_LIGHTRED, "Seu animal de estimação já está spawnado.");
    if(GetPlayerVirtualWorld(playerid) != 0) return SendClientMessage(playerid, COLOR_LIGHTRED, "Você não pode utilizar esse comando em um interior.");
    if(GetPlayerInterior(playerid) != 0) return SendClientMessage(playerid, COLOR_LIGHTRED, "Você não pode utilizar esse comando em um interior.");
    ...
    SendClientMessage(playerid, COLOR_LIGHTRED, "Você spawnou seu animal de estimação %s.", PetData[playerid][petName]);
    PetData[playerid][petTimer] = repeat Pet_Update(playerid, playerid);

    format(logString, sizeof(logString), "%s spawnou seu animal de estimação em %s", PlayerName(targetid, 0), GetPlayerLocation(playerid));
...
PetDespawn(playerid) {
    if(PetData[playerid][petSpawn]) {
        ...
        SendClientMessage(playerid, COLOR_LIGHTRED, "Você desespawnou seu animal de estimação %s.", PetData[playerid][petName]);
        format(logString, sizeof(logString), "%s desespawnou seu animal de estimação em %s", PlayerName(targetid, 0), GetPlayerLocation(playerid));
        logCreate(playerid, logString, 19);
    }
    return true;
}
...
PetSit(playerid) {
    ...
    if(IsValidActor(PetData[playerid][petModel])) {
        ...
        SendClientMessage(playerid, COLOR_LIGHTRED, "Seu animal de estimação agora está sentado.");
    }
    return true;
}
...
PetLay(playerid) {
    ...
    if(IsValidActor(PetData[playerid][petModel]))
    {
        ...
        SendClientMessage(playerid, COLOR_LIGHTRED, "Seu animal de estimação agora está deitado.");
    }
    return true;
}
...
PetJump(playerid) {
    ...
    if(IsValidActor(PetData[playerid][petModel]))
    {
        ...
        SendClientMessage(playerid, COLOR_LIGHTRED, "Seu animal de estimação agora está pulando.");
    }
    return true;
}
...
PetStay(playerid) {
    ...
    if(IsValidActor(PetData[playerid][petModel]))
    {
        ...
        stop PetData[playerid][petTimer];
        ClearActorAnimations(PetData[playerid][petModel]);
        SendClientMessage(playerid, COLOR_LIGHTRED, "Seu animal de estimação agora está parado.");
    }
    return true;
}
...
PetName(playerid) {
    if(PetData[playerid][petSpawn])
        return SendClientMessage(playerid, COLOR_LIGHTRED, "Seu animal de estimação não pode estar spawnado para utilizar isso.");
...
Dialog:PET_NAME(playerid, response, listitem, inputtext[]) {
    if(response) {
        ...
        SendClientMessage(playerid, COLOR_LIGHTRED, "Você definiu o nome do seu animal de estimação como %s.", inputtext);
        format(logString, sizeof(logString), "%s definiu o nome do seu animal de estimação como %s", PlayerName(targetid, 0), inputtext);
        logCreate(playerid, logString, 19);
    }
    return true;
}
...
Dialog:PET_MENU_FOLLOW(playerid, response, listitem, inputtext[]) {
    if(response) {
        ...
        PetFollow(playerid, targetid);
        SendClientMessage(playerid, COLOR_LIGHTRED, "Seu animal de estimação agora está seguindo %s.", pNome(targetid));
